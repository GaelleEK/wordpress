...
class AgenceMenuPage {
    ...
    {
        register_setting(self::GROUP, 'agence_horaire', ['default' => 'horaires non renseignés']);
        register_setting(self::GROUP, 'agence_date');
        add_settings_section('agence_options_section', 'Paramètres', function() {
            echo "Vous pouvez gérer les paramètres liés à l'agence immobilière ici";
        }, self::GROUP);
        ...
        add_settings_field('agence_options_date', "Date d'ouverture", function (){
            ?>
            <input type="text" name="agence_date" value="<?= esc_attr(get_option('agence-date')) ?>" class="montheme_datepicker">
            <?php
        }, self::GROUP, 'agence_options_section');
    }
